...
def build_stage1_model(gru_hidden: int = 256, gru_layers: int = 2, dropout: float = 0.5) -> nn.Module:
    """
    Anomaly Detector — binary classifier (Normal vs Anomaly).

    Architecture matches modelA_detection_clean.ipynb (EfficientGRU) exactly:
        ...
        → Attention pooling
        → Dropout(dropout) → Linear(gru_hidden*2→128) → ReLU → Dropout(dropout*0.5) → Linear(128→2)

    Input:  (B, C, T, H, W)  —  (batch, 3, 16, 224, 224)
    Output: (B, 2)            —  logits for [Normal, Anomaly]
    """
    try:
        import timm
    except ImportError as exc:
        raise ImportError(
            "The anomaly detector requires the 'timm' package. "
    ...
    class EfficientGRU(nn.Module):
        def __init__(self):
            ...
            self.head = nn.Sequential(
                ...
                nn.ReLU(inplace=True),
                nn.Dropout(dropout * 0.5),
                nn.Linear(128, 2),   # logits: [Normal, Anomaly]
            )
